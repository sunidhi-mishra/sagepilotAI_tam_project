...
### ⚙️ [`order-supervisor-simulation (rule-based)`](./order-supervisor-simulation%20\(rule-based\)/)
...
- `README.md` — instructions on how to run it, sample output, a mapping of each part of the build to its Temporal equivalent (signal, timer, query, activity, continue-as-new), and notes on where an LLM could improve the decision step
...
**📁 Folder contents:**
- `order_supervisor_grok.py` — the LLM-integrated simulation code
- `README.md` — setup instructions, how to get an API key, how to run both modes side by side, and what a successful run looks like
- `sample_events.json` — the same input used by the rule-based version, so both can be tested against identical events
- `sample_output.txt` — sample output showing the LLM-driven run, including call counts and fallback counts

**▶️ To run it**, see that folder's own `README.md` for exact setup steps (an API key is required).
